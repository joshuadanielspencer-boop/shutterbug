...
// ---- Added 2026-07-21 -------------------------------------------------
  ...
  // ---- Added 2026-07-23 — broaden the pool so Mr O repeats himself less on any
  // one continent. Sources re-checked 2026-07-23 (Wikipedia/Britannica/UNESCO).
  // ---- Africa ----
  ...
  { q: "On which large island off Africa's coast do almost all the world's wild lemurs live?",
    choices: ["Madagascar", "Sicily", "Sri Lanka", "Zanzibar"], correct: "Madagascar", where: "Africa",
    explain: "Lemurs live in the wild only on Madagascar, which drifted away from Africa long ago." },
  { q: "The source of the Blue Nile is Lake Tana, in which African country?",
    choices: ["Ethiopia", "Egypt", "Sudan", "Uganda"], correct: "Ethiopia", where: "Africa",
    explain: "The Blue Nile begins at Lake Tana in Ethiopia and joins the White Nile in Sudan." },
  // ---- Asia ----
  { q: "Mount Everest, the highest mountain on Earth, sits on the border of Nepal and which country?",
    choices: ["China", "India", "Pakistan", "Bhutan"], correct: "China", where: "Asia",
    explain: "Everest straddles the border between Nepal and China (Tibet)." },
  { q: "The Gobi Desert spreads across northern China and which other country?",
    choices: ["Mongolia", "India", "Kazakhstan", "Thailand"], correct: "Mongolia", where: "Asia",
    explain: "The Gobi Desert covers parts of southern Mongolia and northern China." },
  { q: "Which country now has more people than any other on Earth?",
    choices: ["India", "China", "Indonesia", "Russia"], correct: "India", where: "Asia",
    explain: "India passed China in 2023 to become the most populous country in the world." },
  { q: "Which island country's capital is Tokyo and is nicknamed the Land of the Rising Sun?",
    choices: ["Japan", "South Korea", "Philippines", "Sri Lanka"], correct: "Japan", where: "Asia",
    explain: "Japan, an island country in East Asia, has Tokyo as its capital." },
  // ---- Europe ----
  { q: "Which mountainous country, famous for chocolate and watches, is known for staying neutral in wars?",
    choices: ["Switzerland", "Austria", "Belgium", "Poland"], correct: "Switzerland", where: "Europe",
    explain: "Switzerland, in the Alps, is famous for its long tradition of neutrality." },
  { q: "In which flat, low country would you see fields of tulips, windmills, and land reclaimed from the sea?",
    choices: ["The Netherlands", "Denmark", "Ireland", "Portugal"], correct: "The Netherlands", where: "Europe",
    explain: "Much of the Netherlands lies at or below sea level, kept dry behind dikes." },
  { q: "The northern lights are often seen from which Scandinavian country of deep coastal fjords?",
    choices: ["Norway", "Spain", "Greece", "Italy"], correct: "Norway", where: "Europe",
    explain: "Norway's far north lies under the auroral zone, where the northern lights glow in winter." },
  // ---- North America ----
  { q: "The Grand Canyon in the United States was carved over millions of years by which river?",
    choices: ["The Colorado River", "The Mississippi River", "The Rio Grande", "The Hudson River"], correct: "The Colorado River", where: "North America",
    explain: "The Colorado River cut the Grand Canyon down through the rock of Arizona." },
  { q: "Which Central American country shares its name with a famous canal joining two oceans?",
    choices: ["Panama", "Costa Rica", "Nicaragua", "Honduras"], correct: "Panama", where: "North America",
    explain: "The Panama Canal cuts across Panama, linking the Atlantic and Pacific Oceans." },
  { q: "Greenland, the world's largest island, is a territory of which European country?",
    choices: ["Denmark", "Norway", "Iceland", "Canada"], correct: "Denmark", where: "North America",
    explain: "Greenland is a self-governing territory of Denmark." },
  { q: "The ancient Maya pyramid of Chichén Itzá stands in which country?",
    choices: ["Mexico", "Peru", "Guatemala", "Cuba"], correct: "Mexico", where: "North America",
    explain: "Chichén Itzá, built by the Maya, is on the Yucatán Peninsula of Mexico." },
  // ---- South America ----
  { q: "Which mountain range runs down the whole western side of South America?",
    choices: ["The Andes", "The Rockies", "The Alps", "The Himalayas"], correct: "The Andes", where: "South America",
    explain: "The Andes are the longest mountain range above water, running the length of South America." },
  { q: "Angel Falls, the tallest waterfall on Earth, plunges down in which country?",
    choices: ["Venezuela", "Brazil", "Peru", "Chile"], correct: "Venezuela", where: "South America",
    explain: "Angel Falls, in Venezuela, drops more than 3,000 feet (about 980 m)." },
  { q: "Which South American country is named after the imaginary line that circles the middle of the Earth?",
    choices: ["Ecuador", "Colombia", "Bolivia", "Peru"], correct: "Ecuador", where: "South America",
    explain: "Ecuador means 'equator' in Spanish — the equator runs right through it." },
  // ---- Oceania ----
  { q: "Kangaroos, koalas, and the Great Barrier Reef all belong to which country?",
    choices: ["Australia", "New Zealand", "Fiji", "Indonesia"], correct: "Australia", where: "Oceania",
    explain: "Australia is home to kangaroos and koalas and, off its coast, the Great Barrier Reef." },
  { q: "The Māori are the first people of which Pacific country?",
    choices: ["New Zealand", "Australia", "Fiji", "Samoa"], correct: "New Zealand", where: "Oceania",
    explain: "The Māori settled New Zealand centuries before Europeans arrived." },
  { q: "Which country is made up of two main islands simply called the North and the South?",
    choices: ["New Zealand", "Japan", "Fiji", "Iceland"], correct: "New Zealand", where: "Oceania",
    explain: "New Zealand's two largest islands are named the North Island and the South Island." },
  // ---- Antarctica ----
  { q: "Which birds waddle across the ice of Antarctica and huddle in huge groups against the cold?",
    choices: ["Penguins", "Puffins", "Ostriches", "Flamingos"], correct: "Penguins", where: "Antarctica",
    explain: "Several kinds of penguin live around Antarctica, huddling together to keep warm." },
  { q: "Most of the world's fresh water is locked up in Antarctica as what?",
    choices: ["Ice", "Rivers", "Rain", "Fog"], correct: "Ice", where: "Antarctica",
    explain: "Antarctica's ice sheet holds most of the fresh water on Earth, frozen solid." },
];
...
export const MR_O_FACTS = [
  ...
  // ---- Added 2026-07-23 — more per continent, so Mr O repeats less. Re-checked
  // 2026-07-23 (Wikipedia/Britannica). Any figure leads imperial (rule 3).
  { where: "Africa", text: "The Congo is the deepest river in the world, plunging more than 700 feet (about 220 m) down in places." },
  { where: "Africa", text: "Lake Malawi is home to more kinds of fish than any other lake on Earth." },
  { where: "Asia", text: "The Maldives is the lowest, flattest country on Earth — most of it sits only a few feet above the sea." },
  { where: "Asia", text: "The Rub' al Khali, or 'Empty Quarter,' on the Arabian Peninsula is the largest continuous sand desert on Earth." },
  { where: "Europe", text: "The Vatican, the world's smallest country, has its own post office and a railway station." },
  { where: "Europe", text: "Norway's coast is so folded with fjords and islands that it is one of the longest coastlines of any country." },
  { where: "North America", text: "Death Valley in California is the hottest, driest place in North America, and dips to 282 feet (86 m) below sea level." },
  { where: "North America", text: "Mexico City is slowly sinking, because it was built on the bed of a drained lake." },
  { where: "South America", text: "The Amazon carries more water than the next several largest rivers on Earth put together." },
  { where: "South America", text: "Lake Titicaca, high in the Andes at about 12,500 feet (3,810 m), is the highest lake big boats can sail on." },
  { where: "Oceania", text: "Mainland Australia has no active volcanoes at all." },
  { where: "Oceania", text: "New Zealand has no native land snakes." },
  { where: "Antarctica", text: "Antarctica has no time zones of its own — stations often just use the time of the country that supplies them." },
  { where: null, text: "The Pacific Ocean is so vast that all of Earth's land could fit inside it with room to spare." },
];
